...
def full_frame_queries_tf_idf(idf, stop_list, qry_frames=None, n_frames=3, top_m=5,
                              model_name='kmeans_k=1500.sav', frames_dir='PS4Frames/frames/',
    ...
    for i, qry_frame in enumerate(qry_frames):
        ...
        tf1 = hist1 / np.sum(hist1)
        tf_idf1 = np.array([tf1[key] * val for key, val in idf.items()])
        ...
        for mat_path in mat_paths:
            ...
            tf2 = hist2 / np.sum(hist2)
            tf_idf2 = np.array([tf2[key] * val for key, val in idf.items()])
    ...
    plot_images_grid(all_images, all_names, rows, cols, title='frame_query_tf_idf')


if __name__ == '__main__':

    parser = argparse.ArgumentParser()
    parser.add_argument('--idf_name', type=str, default='idf_k=2000.pkl', help='Path to IDF data')
    parser.add_argument('--stop_list_name', type=str, default='stop_list_0.999.pkl', help='Path to stop list data')
    ...
    idf = joblib.load(args.idf_name)
    stop_list = joblib.load(args.stop_list_name)

    full_frame_queries_tf_idf(idf, stop_list, args.qry_frames, args.n_frames, args.top_m,
                       args.model_name, args.frames_dir, args.sift_dir, args.seed)
